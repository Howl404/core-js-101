...
function isCreditCardNumber(ccn) {
  const ccnStr = String(ccn).replace(/\D/g, '');
  let sum = 0;
  let isDouble = false;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstDirectory.length; i += 1) {
    const currentDirectory = firstDirectory[i];

    for (let j = 1; j < directories.length; j += 1) {
      if (directories[j][i] !== currentDirectory) {
        return commonPath;
      }
    }
